...
class MonthlyCalendar extends Component {
    ...
    render() {
        ...
        return (
            <Frame className="monthly-calendar">
                ...
                <Body className="body">
                    { days_of_the_week.map(weekday => (
                        <WeekDay className="col col-center weekday" key={weekday}>
                            {weekday}
                        </WeekDay>
                    ))}
                    {   generateMonth(currentMonth).map((week, week_index) =>
                            (
                                <div className={`row ${week_index > 4 ? 'long-month' : ''}`} key={week_index}>
                                    {
                                        week.map((day, day_index) => (
                                            day ?
                                            <Day className="col cell day" key={day_index}>
                                            <span className="number">{day ? getDate(day) : ''}</span>
                                            <span className="bg">{getDate(day)}</span>
