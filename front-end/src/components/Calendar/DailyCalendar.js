    ...
    static getDerivedStateFromProps(props, state) {
      ...
      sortableElement(parent_el, tree, hours, i) {
          ...
          if (this.state.view[i] === "basic") {
              ...
              sortableElement = (<div className="hour" style={{height: '100%', display: 'flex', width: '100%'}}>
                  <SortableTree
                    treeData={newTree}
                    onChange={treeData => {
                        // if (i === 0) {
              ...
              sortableElement =
                  hours[i].map((el, i) =>
                   <div className="hour" key={el} style={{height: '68px', display: 'flex', width: '100%'}}>
                      {el !== "general" && <div className="hour-number">{format(el, "ha").toLowerCase()}</div>}
                      <SortableTree
                        key={el+i}
                ...
                <Body className="body">
                    {
                        days.map((parent_el, i) => {
                           let tree = this.state.events[format(parent_el, dayFormat)];
                            return (<Day key={`${parent_el}${i}`} className={`${i%2 ? 'today' : 'outer-days' } day`}>
                                <div className="day-header">
                                    <span className="day-title">{format(parent_el, dayFormat)}</span><div className="view-change">
